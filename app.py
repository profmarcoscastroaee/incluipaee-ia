...
def gerar_docx_documento(conteudo, codigo, tipo="documento"):
    """Gera um arquivo Word (.docx) com o mesmo conteúdo usado no PDF/TXT."""
    try:
        ...
        raise RuntimeError(
            "A biblioteca python-docx não está instalada. Instale com: pip install python-docx"
        ) from e
    ...
    nome_arquivo = f"{prefixo}_{codigo}.docx".replace("/", "-").replace("\\", "-")
    ...
    run = subtitulo.add_run(titulo_doc)
    run.bold = True
    run.font.size = Pt(14)
    ...
    for linha in conteudo.split("\n"):
        ...
        if linha[:2].isdigit() and "." in linha[:4]:
            p = doc.add_paragraph()
            r = p.add_run(linha)
            r.bold = True
            r.font.size = Pt(12)
        elif linha.isupper() and len(linha) > 8:
            p = doc.add_paragraph()
            r = p.add_run(linha)
            r.bold = True
            r.font.size = Pt(12)
    ...
    run = rodape.add_run(f"Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')} pelo INCLUISRM.")
    run.italic = True
    run.font.size = Pt(9)
